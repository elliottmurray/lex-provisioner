...
def put_slot_request(bot_name, slot_name, utterances, plaintext=None):
...
def stub_intent_creation(stubber, put_slot_response, put_request):
    stubber.add_response(
        'put_slot', put_slot_response, put_request)
    stubber.add_response('create_slot_version', {'name': SLOT_NAME,
                                                   'version': '1'},
...
def test_create_slot(put_intent_response,
    ...
    with Stubber(aws_lambda) as lambda_stubber, Stubber(lex) as stubber:
        ...
        intent_builder = SlotBuilder(Mock(), context, lex_sdk=lex, lambda_sdk=aws_lambda)
        plaintext = {
...
def test_create_slot_response(put_slot_response, codehook_uri, mocker,
        lex, aws_lambda, monkeypatch):
    """ test the response from create slot"""
    context = mock_context(mocker)
    monkeypatch_account(monkeypatch)
...
def test_update_slot(put_intent_response, codehook_uri, mocker,
        lex, aws_lambda, monkeypatch):
    context = mock_context(mocker)
